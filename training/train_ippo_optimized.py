"""
Optimized training script for IPPO with improved hyperparameters.
...
def train_ippo_optimized(
    n_episodes: int = 10000,  # Increased from 5000
    n_adversaries: int = 3,
    n_good: int = 1,
    max_cycles: int = 200,
    device: str = "cuda" if torch.cuda.is_available() else "cpu",
    save_dir: str = "checkpoints_ippo_optimized",
    use_lr_scheduler: bool = True,
    warmup_episodes: int = 100
):
    """
    Train IPPO with optimized hyperparameters and advanced features.
    ...
    """
    print("="*70)
    print("OPTIMIZED IPPO Training with Enhanced Hyperparameters")
    ...
    print("  - Learning Rate: 5e-4 (increased from 3e-4)")
    ...
    # Create IPPO agents with optimized hyperparameters (using defaults)
    predator_agent = IPPO(
    ...
    # Create population-based self-play wrapper with reduced update interval
    ...
    for episode in range(n_episodes):
        ...
        if avg_prey_reward > best_prey_reward:
            ...
            episodes_without_improvement = 0
            # Save best model
            prey_agent.save(f"{save_dir}/ippo_prey_best.pt")
        else:
        ...
        if (episode + 1) % 50 == 0:  # More frequent logging
            ...
            print(f"  Population Sizes: Pred={pop_stats['predator_population_size']}, "
                  f"Prey={pop_stats['prey_population_size']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train IPPO with optimized hyperparameters")
    ...
    parser.add_argument("--save-dir", type=str, default="checkpoints_ippo_optimized", help="Save directory")
