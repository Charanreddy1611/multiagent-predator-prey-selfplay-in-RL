"""
Fair comparison between MAPPO and IPPO algorithms.
Both use the same:
...
def train_algorithm(
    ...
    prey_agent = algorithm_class(
        ...
        n_agents=n_good,
        lr=7e-4,  # Higher LR for prey (40% faster learning)
        entropy_coef=0.08,  # More exploration for prey (vs 0.05 default)
        device=device
    )
    ...
    # For MAPPO/IPPO: they have actor_optimizers list and critic_optimizer/optimizers
    ...
    # Use SAME self-play strategy for fair comparison: Alternating
    ...
    # CRITICAL: Warn about configuration
    if n_adversaries == 1 and n_good == 1:
        print("\n" + "🚨"*40)
        print("CRITICAL WARNING: You are running 1v1!")
        print("MAPPO will perform TERRIBLY in 1v1 scenarios!")
        print("Expected prey reward: -1500 to -2500 (catastrophic)")
        print("\nYou should STOP THIS and run 2v2 instead:")
        print("  python training/compare_mappo_vs_ippo.py --adversaries 2 --good 2")
        print("🚨"*40 + "\n")
    ...
    for episode in range(n_episodes):
        ...
        # Logging (changed to 50 episodes for faster feedback)
        if (episode + 1) % 50 == 0:
            ...
            print(f"  Episode Length:  {avg_stats['avg_episode_length']:.1f}")
...
def plot_comparison(mappo_metrics, ippo_metrics, save_path):
    ...
    fig.suptitle('MAPPO vs IPPO Comparison (Same Self-Play Strategy)', 
...
def print_comparison_summary(mappo_metrics, ippo_metrics):
    ...
    print("\n" + "="*80)
    print("COMPARISON SUMMARY: MAPPO vs IPPO")
    print("="*80)
    ...
    if abs(prey_diff) < 5:
        ...
        print("  → MAPPO's centralized critic helps coordination")
    else:
        print(f"  IPPO wins by {abs(prey_diff):.2f} points in prey reward")
        print("  → IPPO's independent learning is more robust")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Compare MAPPO vs IPPO")
    ...
    parser.add_argument("--save-dir", type=str, default="comparison_results", help="Save directory")
    ...
    if args.adversaries == 1 and args.good == 1:
        ...
        print("="*80)
        print("MAPPO is designed for MULTI-AGENT coordination and will perform")
        print("poorly in 1v1 scenarios because:")
        print("  • Centralized critic has no coordination advantage with 1 agent")
        print("  • Team-based reward averaging doesn't help with single agents")
        print("  • MAPPO's strength is in coordinating multiple agents")
        print("\nRecommended configurations for fair comparison:")
        print("  • 2v2: python training/compare_mappo_vs_ippo.py --adversaries 2 --good 2")
        print("  • 3v2: python training/compare_mappo_vs_ippo.py --adversaries 3 --good 2")
        print("  • 2v1: python training/compare_mappo_vs_ippo.py --adversaries 2 --good 1")
        print("\nContinuing with 1v1, but expect IPPO to significantly outperform MAPPO...")
        print("="*80)
        print("⚠️ "*40 + "\n")
        input("Press Enter to continue or Ctrl+C to cancel...")
    
    print("\n" + "="*80)
    print("FAIR COMPARISON: MAPPO vs IPPO")
    print("="*80)
    print(f"Configuration: {args.adversaries} Predator(s) vs {args.good} Prey")
    print(f"Episodes: {args.episodes}")
    print(f"Self-Play Strategy: Alternating (Same for both)")
    print(f"Hyperparameters: Optimized (Same for both)")
    print(f"Device: {args.device}")
    if args.adversaries > 1 or args.good > 1:
        print(f"✓ Multi-agent scenario: Good for MAPPO comparison")
    print("="*80)
    ...
    # Create plots
    plot_path = f"{save_dir}/comparison_plot.png"
    ...
    print(f"✓ Summary: {save_dir}/comparison_summary.json")
