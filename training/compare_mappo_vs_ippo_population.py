"""
Fair comparison between MAPPO and IPPO algorithms using Population-Based Self-Play.
Both use the same:
...
def train_algorithm(
    ...
    """
    Train a single algorithm with population-based self-play and return metrics.
    ...
    Args:
        ...
        update_interval: Episodes between population updates
    
    Returns:
        MetricsTracker with training history
    """
    print("\n" + "="*70)
    print(f"Training {algorithm_name} with Population-Based Self-Play")
    print("="*70)
    ...
    # Use Population-Based self-play strategy
    self_play = PopulationSelfPlay(
        predator_agent=predator_agent,
        prey_agent=prey_agent,
        population_size=population_size,
        update_interval=update_interval,
        sampling_strategy="fitness"  # Use fitness-based sampling
    )
    ...
    # Training loop
    print(f"Starting {algorithm_name} training with Population-Based Self-Play...")
    for episode in range(n_episodes):
        ...
        if (episode + 1) % 100 == 0:
            ...
            print(f"  Population Size: Pred={pop_stats['predator_population_size']}, Prey={pop_stats['prey_population_size']}")
    ...
    print(f"  Final Population Sizes: Pred={pop_stats['predator_population_size']}, Prey={pop_stats['prey_population_size']}")
...
def plot_comparison(mappo_metrics, ippo_metrics, save_path):
    ...
    fig.suptitle('MAPPO vs IPPO Comparison (Population-Based Self-Play)', 
...
def print_comparison_summary(mappo_metrics, ippo_metrics):
    ...
    print("\n" + "="*80)
    print("COMPARISON SUMMARY: MAPPO vs IPPO (Population-Based Self-Play)")
    print("="*80)
    ...
    if abs(prey_diff) < 5:
        ...
        print("  → MAPPO's centralized critic helps with population diversity")
    else:
        print(f"  IPPO wins by {abs(prey_diff):.2f} points in prey reward")
        print("  → IPPO's independent learning adapts better to diverse opponents")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Compare MAPPO vs IPPO with Population-Based Self-Play")
    ...
    parser.add_argument("--save-dir", type=str, default="comparison_population", help="Save directory")
    ...
    parser.add_argument("--update-interval", type=int, default=250, help="Population update interval")
    ...
    print("\n" + "="*80)
    print("FAIR COMPARISON: MAPPO vs IPPO (Population-Based Self-Play)")
    print("="*80)
    print(f"Configuration: {args.adversaries} Predator(s) vs {args.good} Prey")
    print(f"Episodes: {args.episodes}")
    print(f"Self-Play Strategy: Population-Based (Same for both)")
    ...
    # Train MAPPO
    print("\n" + "🔵 " + "="*75)
    print("TRAINING MAPPO WITH POPULATION-BASED SELF-PLAY")
    print("="*78)
    ...
    # Create plots
    plot_path = f"{save_dir}/comparison_plot_population.png"
    ...
    with open(f"{save_dir}/comparison_summary_population.json", 'w') as f:
        json.dump(comparison_data, f, indent=2)
    
    print(f"\n✓ All results saved to {save_dir}/")
    print(f"✓ Comparison plot: {plot_path}")
    print(f"✓ Summary: {save_dir}/comparison_summary_population.json")
